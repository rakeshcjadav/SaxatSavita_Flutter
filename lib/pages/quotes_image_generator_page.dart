...
class _QuotesImageGeneratorPageState extends State<QuotesImageGeneratorPage>
  ...
  // Removed unused template preview section
  /*
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Padding(
            ...
            child: GestureDetector(
              onTap: () => setState(() => _selectedTemplate = templateIndex),
              child: Container(
                width: 90,
                decoration: BoxDecoration(
                  borderRadius: BorderRadius.circular(12),
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Container(
                        ...
                        child: _buildMiniatureTemplate(templateIndex),
                      ),
                    ),
                    // Template name
                    Container(
  ...
  Widget _buildMiniatureTemplate(int templateIndex) {
    final miniatureQuote =
        _quoteController.text.isNotEmpty
            ? _quoteController.text.length > 50
                ? '${_quoteController.text.substring(0, 47)}...'
                : _quoteController.text
            : 'Sample Quote';
    ...
    return ClipRRect(
      ...
      child: Stack(
        children: [
          // Background patterns
          if (templateIndex == 1) _buildMiniGeometricPattern(),
          if (templateIndex == 3) _buildMiniSpiritualPattern(),
          if (templateIndex == 4) _buildMiniMandalaPattern(),
          if (templateIndex == 8) _buildMiniProfilePattern(),
          if (templateIndex == 10) _buildMiniSocialPattern(),

          // Content based on template
          _buildMiniatureContent(templateIndex, miniatureQuote),
        ],
      ),
    );
  }

  Widget _buildMiniatureContent(int templateIndex, String quote) {
    switch (templateIndex) {
      case 8: // Profile
        return Padding(
      ...
      default: // Simple, Geometric, Floral, Spiritual, Mandala, Elegant, Modern, Classic
        return Padding(
          padding: const EdgeInsets.all(8),
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              if (templateIndex == 3) // Spiritual
                Icon(
                  Icons.self_improvement,
                  size: 8,
                  color: Colors.white.withOpacity(0.8),
                ),
              if (templateIndex == 3) const SizedBox(height: 4),
              ...
              Text(
                ...
                style: TextStyle(
                  color: Colors.white,
                  fontSize:
                      templateIndex == 6 ? 5.5 : 6, // Modern slightly smaller
                  fontWeight:
                      templateIndex == 6 ? FontWeight.w300 : FontWeight.w500,
                  fontStyle:
                      templateIndex == 5
                          ? FontStyle.italic
                          : FontStyle.normal, // Elegant italic
                ),
                textAlign:
                    templateIndex == 5
                        ? TextAlign.left
                        : TextAlign.center, // Elegant left-aligned
                maxLines: 4,
                overflow: TextOverflow.ellipsis,
              ),
            ],
  ...
  */
  ...
  Widget _buildMiniGeometricPattern() {
    return CustomPaint(
      painter: MiniGeometricPatternPainter(Colors.white.withOpacity(0.1)),
      child: Container(),
    );
  }

  Widget _buildMiniSpiritualPattern() {
    return Positioned.fill(
      child: CustomPaint(
        painter: MiniSpiritualPatternPainter(Colors.white.withOpacity(0.08)),
        child: Container(),
      ),
    );
  }

  Widget _buildMiniMandalaPattern() {
    return CustomPaint(
      painter: MiniMandalaPatternPainter(Colors.white.withOpacity(0.1)),
      child: Container(),
    );
  }

  Widget _buildMiniProfilePattern() {
    return CustomPaint(
      painter: MiniProfilePatternPainter(Colors.white.withOpacity(0.05)),
      child: Container(),
    );
  }

  Widget _buildMiniSocialPattern() {
    return CustomPaint(
      painter: MiniSocialPatternPainter(Colors.white.withOpacity(0.04)),
      child: Container(),
    );
  }
